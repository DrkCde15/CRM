...
// via webhook para o backend do CRM (FastAPI). Expõe API REST para envio.
const {
  makeWASocket,
...
async function startBot() {
  if (isConnecting) { logger.warn('already_connecting_skipping'); return; }
  isConnecting = true;
  ...
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (qr) { logger.info('qr_code_generated'); qrcode.generate(qr, { small: true }); }
      if (connection === 'open') {
        isConnecting = false;
      ...
      while (webhookQueue.length > 0) {
        ...
        for (let attempt = 0; attempt <= delays.length; attempt++) {
          try {
            ...
            const errInfo = { message: err.message, code: err.code, status: err.response?.status };
    ...
    async function downloadBase64(msg, kind) {
      try {
        const stream = await downloadContentFromMessage(msg.message[kind === 'image' ? 'imageMessage' : 'audioMessage'], kind);
        const chunks = [];
        for await (const chunk of stream) chunks.push(chunk);
        return Buffer.concat(chunks).toString('base64');
      } catch (err) {
        logger.error({ error: err.message }, `${kind}_download_failed`);
        return null;
      }
    }
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      for (const msg of messages) {
        ...
        if (msgType === 'image') imageBase64 = await downloadBase64(msg, 'image');
        let audioBase64 = null;
        let audioMimetype = null;
        if (msgType === 'audio') {
          audioBase64 = await downloadBase64(msg, 'audio');
          if (audioBase64) audioMimetype = msg.message.audioMessage?.mimetype || 'audio/ogg';
        }
...
process.on('SIGINT', () => { logger.info('shutting_down'); process.exit(0); });
