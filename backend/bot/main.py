...
from config import GATEWAY_URL, WHITELIST_ENABLED, WHITELIST, GROUP_ENABLED
from database import init_db, get_db, SessionLocal, Cliente, Conversa, Agendamento, Proposta, get_config, set_config
from handlers.menu import get_menu_text
from handlers.ai import ask_ai, ask_ai_with_image, transcribe_audio, get_model, is_configured as ai_configured
...
def responder_comando_rapido(texto: str) -> str | None:
    # Comandos curtos para acelerar o atendimento do cliente.
    comando = (texto or '').strip().lower()
    if not comando:
        return None

    if comando in {'/menu', 'menu', '/inicio', 'inicio'}:
        return get_menu_text('inicio')
    if comando in {'/ajuda', 'ajuda', '/help', 'help'}:
        return (
            '🤖 *Comandos rápidos*\n\n'
            '• /menu → mostra o menu principal\n'
            '• /solucoes → mostra os serviços e soluções\n'
            '• /site → fala sobre criação de sites\n'
            '• /app → fala sobre apps e plataformas\n'
            '• /automacao → fala sobre automação\n'
            '• /dados → fala sobre dados e dashboards\n'
            '• /contato → informa contato e suporte\n'
            '• /orcamento → solicita orçamento\n'
            '• /reuniao → agenda uma reunião'
        )
    if comando in {'/solucoes', '/solucao', 'solucoes', 'solucao'}:
        return '💡 *Soluções:* sites, apps, automação, UX/UI, análise de dados, dashboards, APIs e integrações.'
    if comando in {'/site', '/sites', 'site', 'sites'}:
        return '🌐 Criamos sites institucionais, landing pages, plataformas e presença digital para vender melhor.'
    if comando in {'/app', '/apps', 'app', 'apps'}:
        return '📱 Desenvolvemos apps web e mobile com foco em experiência, usabilidade e qualidade de entrega.'
    if comando in {'/automacao', '/automacao', 'automacao', 'automação', 'automacoes', 'automações'}:
        return '⚙️ Automatizamos processos, integrações e fluxos para reduzir esforço manual e ganho de produtividade.'
    if comando in {'/dados', '/dashboard', 'dados', 'dashboard', 'dashboards'}:
        return '📊 Criamos dashboards, relatórios e soluções baseadas em dados para apoiar decisões estratégicas.'
    if comando in {'/orcamento', '/orçamento', 'orcamento', 'orçamento'}:
        return '💰 Para solicitar um orçamento, digite *6* no menu principal e descreva sua necessidade.'
    if comando in {'/contato', '/whatsapp', 'contato', 'whatsapp'}:
        return '📞 *Contato:* contato@ryotech.com.br | suporte@ryotech.com.br'
    if comando in {'/agendar', 'agendar'}:
        return '📅 Para agendar uma reunião/demo, digite *2* ou *7* no menu principal.'
    if comando in {'/reuniao', 'reuniao'}:
        return '📋 Para agendar uma reunião, digite *2* no menu principal.'

    if comando.startswith('/'):
        return (
            '⚠️ Comando não reconhecido.\n'
            'Use /menu, /solucoes, /site, /app, /automacao, /dados, /orcamento ou /reuniao.'
        )

    return None


def responder_automatica(texto: str) -> str | None:
    # Respostas automáticas para saudações e dúvidas frequentes.
    mensagem = (texto or '').strip().lower()
    if not mensagem:
        return None

    if mensagem in {'oi', 'olá', 'ola', 'bom dia', 'boa tarde', 'boa noite', 'e ai', 'oi tudo bem'}:
        return 'Olá! 👋 Somos a NEXA TECH. Podemos criar *sites*, *apps*, *automações*, *design*, *dashboards* e *APIs* sob medida para sua empresa. Digite *1* para ver nossos serviços, *2* para agendar uma reunião ou *3* para falar com o bot.'
    if mensagem in {'obrigado', 'valeu', 'muito obrigado', 'agradecido'}:
        return 'Por nada! 😊 Estamos à disposição para transformar sua ideia em solução digital. Digite *0* ou *menu* para voltar ao atendimento.'
    if 'site' in mensagem or 'landing' in mensagem or 'institucional' in mensagem:
        return '🌐 Criamos sites institucionais, landing pages e presença digital para elevar sua marca e transformar visitas em oportunidades.'
    if 'app' in mensagem or 'mobile' in mensagem or 'plataforma' in mensagem:
        return '📱 Podemos desenvolver apps web e mobile, além de plataformas personalizadas para atender seu modelo de negócio.'
    if 'automacao' in mensagem or 'automação' in mensagem or 'processo' in mensagem:
        return '⚙️ Uma automação bem feita reduz tempo, eliminação de erros e melhora a operação da sua empresa.'
    if 'design' in mensagem or 'ux' in mensagem or 'ui' in mensagem:
        return '🎨 O design certo melhora experiência, clareza e conversão. Podemos criar interfaces que conectam produto e usuário.'
    if 'dados' in mensagem or 'dashboard' in mensagem or 'bi' in mensagem:
        return '📊 Nossa área de dados transforma informação em decisão com dashboards, relatórios e análise prática.'
    if 'api' in mensagem or 'integracao' in mensagem or 'integração' in mensagem:
        return '🔌 Trabalhamos com APIs e integrações para conectar sistemas, aplicações e plataformas de forma eficiente.'
    if 'orcamento' in mensagem or 'preço' in mensagem or 'valor' in mensagem:
        return '💰 Podemos montar uma proposta sob medida para seu projeto. Digite *6* para solicitar orçamento ou fale com um atendente.'
    if 'contato' in mensagem or 'whatsapp' in mensagem or 'telefone' in mensagem or 'email' in mensagem:
        return '📞 Fale com a gente por *contato@ryotech.com.br* ou *suporte@ryotech.com.br*.'
    if 'tchau' in mensagem or 'adeus' in mensagem:
        return 'Até logo! 😊 Quando precisar, é só chamar com *Olá* ou *menu*.'
    if 'solucao' in mensagem or 'soluções' in mensagem or 'software' in mensagem or 'sistema' in mensagem:
        return '🛠️ Podemos ajudar com sites, apps, automações, design, dados e APIs sob medida. Digite *1* para ver mais detalhes ou *2* para agendar uma reunião.'
    return None
...
async def processar_menu(telefone: str, texto: str, cliente: Cliente, db: Session) -> str | None:
    ...
    if texto_clean in {'/menu', 'menu', '/inicio', 'inicio'}:
    ...
    if cliente.estado == 'chamado_confirmar':
        if texto_clean == '1':
            titulo = cliente.dados.get('titulo', '')
            descricao = cliente.dados.get('descricao', '')

            # Abre chamado no CRM (Taky) para acompanhamento comercial
            task_id = await create_task(titulo, descricao, telefone)

            # Gera a proposta comercial automaticamente e envia ao cliente
            proposta = await gerar_proposta(titulo, descricao, cliente.nome)
            if proposta:
                db.add(Proposta(
                    telefone=telefone,
                    titulo=titulo,
                    ...
                    modelo=await get_model(),
                ))
                db.commit()
                await send_whatsapp(telefone, f'📝 *Proposta comercial*\n\n{proposta}')

            await update_cliente_estado(telefone, 'chamado_sucesso', db=db)
            sucesso = get_menu_text('chamado_sucesso')
            if proposta:
                sucesso += '\n\n✨ Enviamos acima uma *proposta comercial inicial* para o seu projeto. Digite *0* para voltar ao menu.'
            elif task_id:
                sucesso += '\n\n⚠️ Não foi possível gerar a proposta agora. Nossa equipe entrará em contato em breve.'
            else:
...
async def processar_webhook_async(from_number: str, text: str, msg_type: str, image_base64: str | None = None, audio_base64: str | None = None, mimetype: str | None = None):
    ...
    try:
        ...
        if from_number.endswith('@g.us'):
            group_enabled = group_enabled_effective(db)
        ...
        # Se for áudio, transcreve e depois processa o texto transcrito
            ...
            # Comandos curtos e respostas automáticas têm prioridade sobre a IA.
            resposta_rapida = responder_comando_rapido(text)
            if resposta_rapida is not None:
                resposta = resposta_rapida
            else:
                resposta_automatica = responder_automatica(text)
                if resposta_automatica is not None:
                    resposta = resposta_automatica
                elif re.match(r'^[0-9]$', text) or text.lower() in ('olá', 'oi', 'ola', 'bom dia', 'boa tarde', 'boa noite', 'menu', 'inicio', '0', '/menu'):
        ...
        # Espelha no CRM (crm.db) para manter o Inbox unificado. Falha aqui
        # não deve quebrar o atendimento.
        try:
            from crm_bridge import bridge_to_crm
            bridge_to_crm(from_number, text, resposta, msg_type)
        except Exception as exc:
            logger.warning('crm_bridge_failed', from_number=from_number, error=str(exc))
...
async def webhook(request: Request, payload: WebhookPayload, background_tasks: BackgroundTasks):
    ...
    db = SessionLocal()
    try:
        whitelist = whitelist_effective(db)
        ...
        whitelist_enabled = whitelist_enabled_effective(db)
...
def _env_flag(value, default: bool = False) -> bool:
    # Interpreta valores tipo booleano de variáveis de ambiente.
    if value is None:
        return default
    return str(value).strip().lower() in ('true', '1', 'yes', 'sim', 's', 'on')


def whitelist_enabled_effective(db: Session = None) -> bool:
    # Env precede o banco quando definida.
    if WHITELIST_ENABLED is not None:
        return _env_flag(WHITELIST_ENABLED)
    return get_config('whitelist_enabled', '1', db) == '1'


def whitelist_effective(db: Session = None) -> set:
    # Env precede o banco quando definida e não vazia.
    if WHITELIST and WHITELIST.strip():
        return {n.split('@')[0] if '@' in n else n for n in WHITELIST.split(',') if n.strip()}
    return _get_whitelist(db)


def group_enabled_effective(db: Session = None) -> bool:
    # Env precede o banco quando definida.
    if GROUP_ENABLED is not None:
        return _env_flag(GROUP_ENABLED)
    return get_config('group_enabled', '0', db) == '1'
...
@app.get('/propostas')
async def list_propostas(db: Session = Depends(get_db)):
    # Lista todas as propostas de orçamento geradas automaticamente (endpoint público)
    propostas = db.query(Proposta).order_by(Proposta.created_at.desc()).all()
    return [
        {
            'id': p.id,
            'telefone': p.telefone,
            ...
            'proposta_texto': p.proposta_texto,
            'modelo': p.modelo,
...
if __name__ == '__main__':
    import uvicorn
    uvicorn.run('main:app', host='0.0.0.0', port=int(os.getenv('BOT_PORT', '8001')), reload=False)
