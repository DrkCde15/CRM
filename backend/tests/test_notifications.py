def test_notifications_flow(client):
    ...
    r = client.post("/api/auth/login", data={"username": "notif@crm.com", "password": "Secret123"})
...
def test_mark_all_read(client):
    ...
    r = client.post("/api/auth/login", data={"username": "notif2@crm.com", "password": "Secret123"})
    token = r.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}

    client.post("/api/tickets", json={"titulo": "A", "tipo": "chamado"}, headers=headers)
    client.post("/api/tickets", json={"titulo": "B", "tipo": "chamado"}, headers=headers)
